...
export function convertCssUnits(
  ...
  console.log(
    propertyName + ": " + currentValue + " " + currentUnit + " → " + convertUnit
  );
  //
  const canvasWrapper = new CanvasWrapper();
  ...
  thisCurrentUnit = vwUnits.some((unit) => unit === thisCurrentUnit)
    ? "VW"
    : thisCurrentUnit;
  thisCurrentUnit = vhUnits.some((unit) => unit === thisCurrentUnit)
    ? "VH"
    : thisCurrentUnit;
  thisConvertUnit = vwUnits.some((unit) => unit === thisConvertUnit)
    ? "VW"
    : thisConvertUnit;
  thisConvertUnit = vhUnits.some((unit) => unit === thisConvertUnit)
    ? "VH"
    : thisConvertUnit;
  ...
  switch (caseValue) {
    ...
    case "PX-%":
      ...
      returnValue = Math.round(
        pxToPercentage(propertyName, currentValue, canvasWrapper)
      );
    ...
    case "PX-REM":
      returnValue =
        Math.round(pxToRem(currentValue, canvasWrapper) * 100) / 100;
      return { values: returnValue, unit: convertUnit };
    // px to ch
    case "PX-CH":
    ...
    case "%-PX":
      returnValue = Math.round(
        percentageToPx(propertyName, currentValue, canvasWrapper)
      );
      return { values: returnValue, unit: convertUnit };
    // % to em
    case "%-EM":
      returnValue =
        Math.round(
          percentageToEm(propertyName, currentValue, canvasWrapper) * 100
        ) / 100;
      return { values: returnValue, unit: convertUnit };
    // % to rem
    case "%-REM":
      returnValue =
        Math.round(
          percentageToRem(propertyName, currentValue, canvasWrapper) * 100
        ) / 100;
      return { values: returnValue, unit: convertUnit };
    // % to ch
    case "%-CH":
      returnValue =
        Math.round(
          percentageToCh(propertyName, currentValue, canvasWrapper) * 100
        ) / 100;
      return { values: returnValue, unit: convertUnit };
    // % to vw
    case "%-VW":
      returnValue =
        Math.round(
          percentageToVw(propertyName, currentValue, canvasWrapper) * 100
        ) / 100;
      return { values: returnValue, unit: convertUnit };
    // % to vh
    case "%-VH":
      returnValue =
        Math.round(
          percentageToVh(propertyName, currentValue, canvasWrapper) * 100
        ) / 100;
      return { values: returnValue, unit: convertUnit };
    // em to px
    case "EM-PX":
    ...
    case "EM-%":
      returnValue = Math.round(
        emToPercentage(propertyName, currentValue, canvasWrapper)
      );
      return { values: returnValue, unit: convertUnit };
    // em to rem
    case "EM-REM":
      returnValue =
        Math.round(emToRem(currentValue, canvasWrapper) * 100) / 100;
      return { values: returnValue, unit: convertUnit };
    // em to ch
    case "EM-CH":
    ...
    case "REM-%":
      returnValue = Math.round(
        remToPercentage(propertyName, currentValue, canvasWrapper)
      );
      return { values: returnValue, unit: convertUnit };
    // rem to em
    case "REM-EM":
      returnValue =
        Math.round(remToEm(currentValue, canvasWrapper) * 100) / 100;
      return { values: returnValue, unit: convertUnit };
    // rem to ch
    case "REM-CH":
      returnValue =
        Math.round(remToCh(currentValue, canvasWrapper) * 100) / 100;
      return { values: returnValue, unit: convertUnit };
    // rem to vw
    case "REM-VW":
      returnValue =
        Math.round(remToVw(currentValue, canvasWrapper) * 100) / 100;
      return { values: returnValue, unit: convertUnit };
    // rem to vh
    case "REM-VH":
      returnValue =
        Math.round(remToVh(currentValue, canvasWrapper) * 100) / 100;
      return { values: returnValue, unit: convertUnit };
    // ch to px
    case "CH-PX":
    ...
    case "CH-%":
      returnValue = Math.round(
        chToPercentage(propertyName, currentValue, canvasWrapper)
      );
      return { values: returnValue, unit: convertUnit };
    // ch to em
    case "CH-EM":
    ...
    case "CH-REM":
      returnValue =
        Math.round(chToRem(currentValue, canvasWrapper) * 100) / 100;
      return { values: returnValue, unit: convertUnit };
    // ch to vw
    case "CH-VW":
    ...
    case "VW-%":
      returnValue = Math.round(
        vwToPercentage(propertyName, currentValue, canvasWrapper)
      );
      return { values: returnValue, unit: convertUnit };
    // vw to em
    case "VW-EM":
    ...
    case "VW-REM":
      returnValue =
        Math.round(vwToRem(currentValue, canvasWrapper) * 100) / 100;
      return { values: returnValue, unit: convertUnit };
    // vw to ch
    case "VW-CH":
    ...
    case "VH-%":
      returnValue = Math.round(
        vhToPercentage(propertyName, currentValue, canvasWrapper)
      );
      return { values: returnValue, unit: convertUnit };
    // vh to em
    case "VH-EM":
    ...
    case "VH-REM":
      returnValue =
        Math.round(vhToRem(currentValue, canvasWrapper) * 100) / 100;
      return { values: returnValue, unit: convertUnit };
    // vh to ch
    case "VH-CH":
...
function pxToVw(pxValue, canvasWrapper) {
  ...
  const viewportWidth =
    ...
    canvasDocument.documentElement.clientWidth ||
    canvasDocument.body.clientWidth;
...
function pxToVh(pxValue, canvasWrapper) {
  ...
  const viewportHeight =
    ...
    canvasDocument.documentElement.clientHeight ||
    canvasDocument.body.clientHeight;
...
function ifHeightProperty(propertyName) {
  ...
  const isHeightProperty = heightProperties.some((unit) =>
    unit.includes(propertyName)
  );
