...
export class CssStyleManager {
  constructor() {
    ...
    for (let i = 0; i < this.styleSheets.length; i++) {
      if (this.styleSheets[i].href.endsWith(this.defaultCssFileName)) {
        ...
        this.defaultRules =
          this.defaultStyleSheet.cssRules || this.defaultStyleSheet.rules;
  ...
  getAppliedRule(propertyName) {
    ...
    if (propertyValue !== "") {
      const isImportant = this.isImportant(
        this.selectedElm.style,
        propertyName
      );
    ...
    if (appliedPropertyValue === null) {
      ...
      for (let i = 0; i < objectKeys.length / 2; i++) {
        ...
        if (selectorText === selectorToMatch) {
          ...
          const propertyValue =
            this.effectiveRules[stringKey].style[propertyName];

          // if propertyValue is not empty, check if !important is set
          if (propertyValue !== "") {
            const propertySpecificity =
              this.effectiveRules[stringKey].specificity;
            const isImportant = this.isImportant(
              this.effectiveRules[stringKey].style,
              propertyName
            );
    ...
    if (appliedPropertyValue === null) {
      ...
      for (let i = 0; i < objectKeys.length / 2; i++) {
        ...
        const propertyValue =
          this.effectiveDefaultRules[stringKey].style[propertyName];
        if (propertyValue !== "") {
          const propertySpecificity =
            this.effectiveDefaultRules[stringKey].specificity;
          const isImportant = this.isImportant(
            this.effectiveDefaultRules[stringKey].style,
            propertyName
          );
    ...
    if (selector === "inline") {
      ...
      if (propertyValue !== "") {
        ...
        isImportant = this.isImportant(
          this.effectiveRules[selector].style,
          propertyName
        );
  ...
  getSpecificity(selectorText) {
    ...
    const specificityValue =
      specificityArray[0] * 1000 +
      specificityArray[1] * 100 +
      specificityArray[2] * 10 +
      specificityArray[3];
