  ...
  elm.addEventListener(
    ...
    (event) => {
      ...
      if (event.keyCode === 46 || event.keyCode === 8) {
        ...
        if (
          selectedElm.tagName !== "BODY" &&
          selectedElm.hasAttribute("contenteditable") === false
        ) {
      ...
      if (event.metaKey && event.key === "Enter") {
        ...
        if (
          selectedElm.tagName !== "BODY" &&
          selectedElm.hasAttribute("contenteditable") === true
        ) {
    ...
    (event) => {
      ...
      if (
        event.target.hasAttribute(
          canvasWrapper.getSelectedElementAttributeName()
        ) ||
        ignoreId.includes(event.target.id)
      ) {
        return;
      }
...
export function addClickEventListeners(elm) {
  ...
  elm.addEventListener(
    ...
    (event) => {
      ...
      let targetElements = canvasDocument.querySelector(
        '[contenteditable="true"]'
      );
      if (targetElements !== null) {
        if (
          elm.getAttribute("data-aperta-uuid") !==
          targetElements.getAttribute("data-aperta-uuid")
        ) {
      ...
      const hoverCaption =
        event.target.ownerDocument.getElementById("hoverCaption");
...
function addListenersToChangeSizeMarginPadding() {
  ...
  canvasDocument.addEventListener("mousemove", function (event) {
    ...
    const marginRight = parseInt(
      window.getComputedStyle(selectedElm).marginRight
    );
    const marginBottom = parseInt(
      window.getComputedStyle(selectedElm).marginBottom
    );
    const marginLeft = parseInt(
      window.getComputedStyle(selectedElm).marginLeft
    );
    ...
    const paddingTop = parseInt(
      window.getComputedStyle(selectedElm).paddingTop
    );
    const paddingRight = parseInt(
      window.getComputedStyle(selectedElm).paddingRight
    );
    const paddingBottom = parseInt(
      window.getComputedStyle(selectedElm).paddingBottom
    );
    const paddingLeft = parseInt(
      window.getComputedStyle(selectedElm).paddingLeft
    );
    ...
    if (marginChanged === true) {
      ...
      marginLine.style.height =
        rectBottom - rectTop + marginLineTop + marginLineBottom + "px";
      marginLine.style.width =
        rectRight - rectLeft + marginLineRight + marginLineLeft + "px";
    ...
    if (paddingChanged === true) {
      ...
      paddingLine.style.height =
        rectBottom - rectTop - paddingLineTop * 2 - paddingLineBottom + "px";
      paddingLine.style.width =
        rectRight - rectLeft - paddingLineRight - paddingLineLeft * 2 + "px";
...
export function addDragAndDropEventListeners(elm) {
  ...
  elm.addEventListener("dragover", (event) => {
    ...
    //
    if (
      event.target.getAttribute("data-aperta-uuid") ===
      draggedElm.getAttribute("data-aperta-uuid")
    ) {
    ...
    if (draggedOverOutline === null) {
      ...
      draggedOverOutline.style.width =
        absoluteRectRight - absoluteRectLeft + "px";
      draggedOverOutline.style.height =
        absoluteRectBottom - absoluteRectTop + "px";
