...
function iniPage() {
    ...
    canvasDocument.addEventListener('keydown', function (event) {
    ...
    fetch('../assets/json/elementList.json')
        ...
        .then(data => {
            ...
            for (let elementGroup of data) {
                ...
                let container = document.querySelector('.' + elementGroup.name + 'Container');
                container.style.display = 'grid';
                container.style.gridTemplateColumns = 'repeat(3, 1fr)';
                container.style.gridTemplateRows = ('repeat(' + (Math.ceil(elementGroup.elements.length / 3)) + ', 1fr)');
...
function getContents() {
    ...
    const selectedOptionId = selectedElement.options[selectedElement.selectedIndex].id;

    const url = "https://cdn.contentful.com/spaces/rvj0gs4gb6wz/environments/master/entries?access_token=4jj76mhuV9M5drJOAwzDx6Lpuv1eN8TLE4DHc6hN4hw&content_type=" + selectedOptionId;
    fetch(url)
        .then(response => response.json())
        .then(entries => {
            console.log(entries);
